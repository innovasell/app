...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    die('<div class="container my-5"><div class="alert alert-danger">ID da formulação não fornecido ou inválido.</div></div>');
}
$id = intval($_GET['id']);

// BUSCAR DADOS DA FORMULAÇÃO PRINCIPAL
$stmt_formula = $conn->prepare("SELECT * FROM formulacoes WHERE id = ?");
$stmt_formula->bind_param("i", $id);
$stmt_formula->execute();
$formula = $stmt_formula->get_result()->fetch_assoc();
if (!$formula) {
    die('<div class="container my-5"><div class="alert alert-danger">Formulação não encontrada.</div></div>');
}

// BUSCAR DADOS RELACIONADOS
// Ativos em Destaque
$stmt_ativos = $conn->prepare("SELECT * FROM ativos_destaque WHERE formulacao_id = ? ORDER BY id ASC");
$stmt_ativos->bind_param("i", $id);
$stmt_ativos->execute();
$ativos = $stmt_ativos->get_result()->fetch_all(MYSQLI_ASSOC);

// Sub-Formulações e seus filhos (Fases e Ingredientes)
...
<div class="container py-4">
    ...
    <?php if (isset($_GET['sucesso_update'])): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <strong>Sucesso!</strong> A formulação foi atualizada.
            ...
            <?php foreach ($sub_formulacoes as $sub): ?>
                <div class="mt-5">
                    ...
                    <?php if (!empty($sub['fases'])): ?>
                                ...
                                <table class="table table-bordered table-striped table-sm">
                                    ...
                                    <tbody>
                                        <?php foreach ($fase['ingredientes'] as $ingrediente): ?>
                                            <tr class="<?php echo ($ingrediente['destaque'] == 1) ? 'table-info fw-bold' : ''; ?>">
                                                <td><?php echo htmlspecialchars($ingrediente['materia_prima']); ?></td>
                                                <td><?php echo htmlspecialchars($ingredente['inci_name']); ?></td>
                                                <td class="text-end">
                                                    <?php 
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-muted">Nenhuma fase cadastrada para esta parte.</p>
                    <?php endif; ?>
