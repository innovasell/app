...
foreach ($files as $arquivo) {
    ...
    foreach ($infNFe->det as $det) {
        ...
        // IBS / CBS
        $cstIBSCBS=''; $cClassTribIBS=''; $bcIBSCBS=0; $pIBSUF=0; $vIBSUF=0; $pIBSMun=0; $vIBSMun=0; $vIBS=0; $pCBS=0; $vCBS=0;
        if(isset($det->imposto->IBSCBS)) {
            $cstIBSCBS = (string) ($det->imposto->IBSCBS->CST ?? '');
            $cClassTribIBS = (string) ($det->imposto->IBSCBS->cClassTrib ?? '');
            $bcIBSCBS = (float) ($det->imposto->IBSCBS->gIBSCBS->vBC ?? 0);
            $pIBSUF = (float) ($det->imposto->IBSCBS->gIBSUF->pIBSUF ?? 0);
            $vIBSUF = (float) ($det->imposto->IBSCBS->gIBSUF->vIBSUF ?? 0);
            $pIBSMun = (float) ($det->imposto->IBSCBS->gIBSMun->pIBSMun ?? 0);
            $vIBSMun = (float) ($det->imposto->IBSCBS->gIBSMun->vIBSMun ?? 0);
            $vIBS = (float) ($det->imposto->IBSCBS->vIBS ?? 0);
            $pCBS = (float) ($det->imposto->IBSCBS->gCBS->pCBS ?? 0);
            $vCBS = (float) ($det->imposto->IBSCBS->gCBS->vCBS ?? 0);
        }

        // Calculated PIS/COFINS (For DB Backwards compatibility)
        ...
        $itens_db[] = [
            ...
            'cstIBSCBS' => $cstIBSCBS, 'cClassTribIBS' => $cClassTribIBS, 'bcIBSCBS' => $bcIBSCBS, 'pIBSUF' => $pIBSUF, 'vIBSUF' => $vIBSUF,
            'pIBSMun' => $pIBSMun, 'vIBSMun' => $vIBSMun, 'vIBS' => $vIBS, 'pCBS' => $pCBS, 'vCBS' => $vCBS,
            'di' => $arrDI
        ];
    }
    ...
    $nfStruct = [
        ...
        'tot_IBSCBSTot_vIBS' => (float)($total->IBSCBSTot->vIBS ?? 0),
    ...
    // Apenas passamos a devolutiva basica para o FRONT e DATATABLES
    // Pra economizar banda do servidor (XHR Limit JSON Array via web) não enviamos a payload gigante pra VIEW
    $stats['nfs'][] = [
        "chNFe"      => $chNFe, "nNF" => $nNF, "dhEmi" => $dhEmi,
        "cnpj_emit"  => $cnpjEmit, "nome_emit" => $nomeEmit,
        "cnpj_dest"  => $cnpjDest, "nome_dest" => $nomeDest,
    ...
    if (!$ja_existe) {
        try {
            ...
            $pdo->rollBack();
            $stats['erros']++;
            $stats['log'][] = ['tipo' => 'erro', 'msg' => "Erro DB na NF $nNF."];
        }
    }
}
